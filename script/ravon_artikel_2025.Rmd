...
temp %>%
  ...
  geom_point(aes(color = cpue == 0), size = 1.5) + # Maak punten wit als CPUE 0 is
  ...
  scale_x_continuous(breaks = seq(min(temp$jaar), max(temp$jaar), by = 1)) + # Alleen hele getallen op x-as
  labs(
    title = "Trend in de vangst per eenheid van inspanning per jaar per vijver",
    x = "Jaar", # x-as label aanpassen
    y = "Totale vangst per fuik per dag" # y-as label aanpassen
  )

  ggsave("./interim/figuren_ravon_artikel/2025/Trend_VPEI_jaar_vijver.png", width = 10, height = 6, units = "in")
