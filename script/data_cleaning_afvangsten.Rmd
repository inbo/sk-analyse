...
test <- afvangsten_per_fuik %>% 
  anti_join(afvangsten_per_fuik_recomb) %>% 
...
afvangsten_per_fuik_combined <- bind_rows(afvangsten_per_fuik, afvangsten_per_fuik_pre2018) %>% 
   mutate(location_temp = as.integer(substr(eventID, 12, 16))) %>% 
  tidylog::distinct(eventID, fuikNr, .keep_all = TRUE) # Exclude overlap between Pre2018 and Post2018 data => in case of overlap, keep the Post2018 data. 
