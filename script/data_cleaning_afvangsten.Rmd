...
Organisation_unknown <- data %>% 
  ...
  select(Tijdstempel, `E-mailadres`, Invuller, `Invuller 2`, Organisation)
...
table(data$`Wat wil je melden`, useNA = "ifany")
table(data$locationID, useNA = "ifany")

missing_locationID <- data %>% 
  filter(is.na(locationID)) %>% 
  distinct(Locatie)
...
data_afvangsten <- data %>% 
  mutate(Jaar = case_when(is.na(Jaar) ~ as.character(lubridate::year(as.Date(Datum, format = "%d-%m-%Y"))),
                          TRUE ~ Jaar),
         Jaar_Tijdstempel = lubridate::year(as.Date(Tijdstempel, format = "%d-%m-%Y %H:%M:%S"))) %>% 
  tidylog::filter(grepl("Afvangst", `Wat wil je melden`)) %>% 
  ...
  mutate(eventID = paste(format(as.Date(Datum, format = "%d-%m-%Y"), "%d-%m-%Y"), Locatie, sep = "_"))
```

```{r}
toekomstige_afvangsten <- data_afvangsten %>% 
  tidylog::filter(Jaar_Tijdstempel < Jaar)

data_afvangsten <- data_afvangsten %>% 
  tidylog::filter(Jaar_Tijdstempel >= Jaar | is.na(Jaar_Tijdstempel))
```

```{r datacleaning: events - long}
# Selecteer events met Aantal fuiken (Totaal) == 1 
# Deze events hebben data op meerdere rijen
# We tellen het aantal rijen op als totaal aantal fuiken
data_afvangsten_long <- data_afvangsten %>% 
  filter(`Aantal fuiken (Totaal)` == 1) %>% 
  group_by(eventID, `Wat wil je melden`) %>%
  mutate(`Aantal fuiken (Totaal)` = n()) %>% 
  ungroup()
...
data_afvangsten_wide <- data_afvangsten %>% 
  filter(`Aantal fuiken (Totaal)` != 1) %>% 
  filter(!eventID %in% data_afvangsten_long_checked$eventID) %>%
  group_by(eventID, `Wat wil je melden`) %>% 
  add_tally() %>%
  ungroup()

# Alle data van het event staat op 1 rij
# Split catch over fuiken evenly & set fuik characteristics to NA
data_afvangsten_wide_single <- data_afvangsten_wide %>%
  filter(n == 1) %>% 
  select(-n)

lifestages <- c("L00", "L0", "L1", "L2", "M1", "M2", "AM", "AV")

data_afvangsten_wide_single_recomb <- data.frame()

for(i in 1:nrow(data_afvangsten_wide_single)){
  temp_afvangsten_wide_single <- data_afvangsten_wide_single[i,]
  
  # split catch evenly
  for(ls in lifestages){
    ls_split <- floor(as.integer(temp_afvangsten_wide_single[[paste0("Fuik 1 - ", ls)]])/temp_afvangsten_wide_single$`Aantal fuiken (Totaal)`) 
    
    ls_diff <- as.integer(temp_afvangsten_wide_single[[paste0("Fuik 1 - ", ls)]]) - ls_split*temp_afvangsten_wide_single$`Aantal fuiken (Totaal)` 
    
    ls_value <- ifelse(!is.na(as.integer(temp_afvangsten_wide_single[[paste0("Fuik 1 - ", ls)]])), as.integer(temp_afvangsten_wide_single[[paste0("Fuik 1 - ", ls)]]), 0)
    ls_sum <- temp_afvangsten_wide_single %>% 
      mutate(across(matches(ls), as.numeric)) %>% 
      mutate(ls_sum = sum(across(matches(ls)), na.rm = TRUE)) %>% 
      pull(ls_sum)
    
    if(ls_value == ls_sum){
      for(n in 1:temp_afvangsten_wide_single$`Aantal fuiken (Totaal)`){
        
        if(n == 1){
          temp_afvangsten_wide_single[[paste0("Fuik ", n, " - ", ls)]] <- as.character(ls_split + ls_diff) 
        }else{
          temp_afvangsten_wide_single[[paste0("Fuik ", n, " - ", ls)]] <- as.character(ls_split)
          temp_afvangsten_wide_single[[paste0("Fuik ", n, " - Hoek")]] <- NA_character_
        }
      }
    }else{
      cat("unexpected value detected \n")
      next
    }
  }
  if(nrow(data_afvangsten_wide_single_recomb) == 0){
    data_afvangsten_wide_single_recomb <- temp_afvangsten_wide_single
  }else{
    data_afvangsten_wide_single_recomb <- rbind(data_afvangsten_wide_single_recomb, 
                                                temp_afvangsten_wide_single)
  }
}
...
data_afvangsten_wide_multiple_1_recomb <- data_afvangsten %>% 
  filter(eventID %in% data_afvangsten_wide_multiple_1$eventID) %>% 
  group_by(eventID, `Wat wil je melden`) %>%
...
data_afvangsten_wide_multiple_2 <- data_afvangsten_wide_multiple %>%
  ...
  ungroup() %>%
  group_by(eventID, `Wat wil je melden`) %>%
  # Keep rows with differing values in target columns OR latest Tijdstempel
  filter(
    if (n_distinct(across(matches("L00|L0|L1|L2|M1|M2|AM|AV"))) > 1) TRUE
    else Tijdstempel == max(Tijdstempel)
  ) %>%
  ungroup() %>%
  select(-flag)

data_afvangsten_wide_multiple_recomb <- rbind(data_afvangsten_wide_multiple_1_recomb,
                                              data_afvangsten_wide_multiple_2) %>% 
  group_by(Tijdstempel, eventID, `Wat wil je melden`) %>%
  filter(`Aantal fuiken (Totaal)` == max(`Aantal fuiken (Totaal)`, na.rm = TRUE)) %>% 
  ungroup()

# combineer data
data_afvangsten_wide <- bind_rows(data_afvangsten_wide_single_recomb,
                                  data_afvangsten_wide_multiple_recomb) 

if("n" %in% names(data_afvangsten_wide)){
  data_afvangsten_wide$n <- NULL
}

data_afvangsten_wide <- data_afvangsten_wide %>% 
  group_by(Tijdstempel, eventID, `Wat wil je melden`) %>% 
  distinct()
...
```{r  datacleaning: events - rest}
data_afvangsten_rest <- data_afvangsten %>% 
  filter(!Tijdstempel %in% data_afvangsten_wide$Tijdstempel) %>% 
  filter(!Tijdstempel %in% data_afvangsten_long_checked$Tijdstempel) %>% 
  filter(!Tijdstempel %in% data_afvangsten_long$Tijdstempel) %>% 
  group_by(eventID, `Wat wil je melden`) %>% 
  add_tally() 

data_afvangsten_rest_uniek <- data_afvangsten_rest %>% 
  filter(n == 1) 
```
...
data_afvangsten <- bind_rows(data_afvangsten_wide,
                             ...
                             data_afvangsten_rest_uniek) %>% 
  select(-n)
```

```{r}
missing_from_recomb <- data %>% 
  filter(!Tijdstempel %in% data_afvangsten$Tijdstempel) %>% 
  filter(grepl("Afvangst", `Wat wil je melden`))
```
...
test0 <- rawdata %>% 
  filter(grepl("Afvangst", `Wat wil je melden`)) %>%
  group_by(Jaar) %>% 
  mutate(across(matches("L0|L00|L1|L2"), as.numeric)) %>% 
  summarise(larvae_raw = sum(across(contains("L0") | contains("L00") | contains("L1") | contains("L2")), na.rm = TRUE))

test1 <- data_afvangsten_long_checked %>% 
  group_by(Jaar) %>% 
  mutate(across(matches("L0|L00|L1|L2"), as.numeric)) %>% 
  summarise(larvae_long = sum(across(contains("L0") | contains("L00") | contains("L1") | contains("L2")), na.rm = TRUE))

test2 <- data_afvangsten_wide %>% 
  group_by(Jaar) %>% 
  mutate(across(matches("L0|L00|L1|L2"), as.numeric)) %>% 
  summarise(larvae_wide = sum(across(contains("L0") | contains("L00") | contains("L1") | contains("L2")), na.rm = TRUE))

test3 <- data_afvangsten_rest_uniek %>% 
  ungroup() %>% 
  group_by(Jaar) %>% 
  mutate(across(matches("L0|L00|L1|L2"), as.numeric)) %>% 
  summarise(larvae_rest = sum(across(contains("L0") | contains("L00") | contains("L1") | contains("L2")), na.rm = TRUE))

test <- test0 %>% 
  full_join(test1, by = "Jaar") %>%
  full_join(test2, by = "Jaar") %>% 
  full_join(test3, by = "Jaar") %>%
  replace_na(replace = list(larvae_long = 0)) %>% 
  replace_na(replace = list(larvae_wide = 0)) %>%
  replace_na(replace = list(larvae_rest = 0)) %>%
  mutate(diff = larvae_raw - (larvae_long + larvae_wide + larvae_rest)) %>% 
  write_csv("./interim/test.csv")
```
...
```{r FUN: prep_fuiken}
prep_fuiken <- function(df){
  df <- df %>% 
    tidylog::left_join(events, by = c("eventID", "Wat wil je melden")) %>% 
    tidylog::filter(!is.na(Tijdstempel)) %>% 
    tidylog::group_by(Datum, Locatie) %>% 
    tidylog::pivot_longer(contains("Fuik", ignore.case = FALSE), 
                          values_to = "value",
                          values_ptypes = character()) %>% 
    tidylog::ungroup() %>% 
    tidylog::filter(!is.na(value)) %>% 
    tidylog::mutate(value = case_when(value == "-1" ~ NA_character_,
                                      TRUE ~ value)) %>% 
    separate(name, c("fuikNr", "type"), " - ",
             convert = TRUE) %>% 
    tidylog::pivot_wider(names_from = type, 
                         values_from = value, 
                         values_fn = max) %>% 
    mutate(fuiktype_base = case_when(grepl(pattern = "dubbele fuik",
                                           x = `Wat wil je melden`,
                                           ignore.case = TRUE) ~ "dubbele fuik",
                                     grepl(pattern = "enkele fuik",
                                           x = `Wat wil je melden`,
                                           ignore.case = TRUE) ~ "enkele fuik",
                                     `Wat wil je melden` == "Afvangst" ~ NA_character_,
                                     `Wat wil je melden` == "Afvangst Vrijwilligers Natuurpunt" ~ NA_character_,
                                     TRUE ~ "--AANVULLEN--")) %>% 
    mutate(fuiktype_info = "(",
           fuiktype_info = ifelse(grepl(pattern = "13m", 
                                        `Wat wil je melden`, 
                                        ignore.case = TRUE), 
                                  paste(fuiktype_info, "13m", sep = " + "), 
                                  fuiktype_info),
           fuiktype_info = ifelse(grepl(pattern = "20m", 
                                        `Wat wil je melden`, 
                                        ignore.case = TRUE), 
                                  paste(fuiktype_info, "20m", sep = " + "), 
                                  fuiktype_info),
           fuiktype_info = ifelse(grepl(pattern = "samengesteld", 
                                        `Wat wil je melden`, 
                                        ignore.case = TRUE), 
                                  paste(fuiktype_info, "samengesteld", sep = " + "), 
                                  fuiktype_info),
           fuiktype_info = ifelse(grepl(pattern = "netinkeel", 
                                        `Wat wil je melden`, 
                                        ignore.case = TRUE), 
                                  paste(fuiktype_info, "netinkeel", sep = " + "), 
                                  fuiktype_info),
           fuiktype_info = ifelse(grepl(pattern = "fijne mazen", 
                                        `Wat wil je melden`, 
                                        ignore.case = TRUE), 
                                  paste(fuiktype_info, "fijne mazen", sep = " + "), 
                                  fuiktype_info),
           fuiktype_info = ifelse(grepl(pattern = "mazen 4 mm", 
                                        `Wat wil je melden`, 
                                        ignore.case = TRUE), 
                                  paste(fuiktype_info, "mazen 4 mm", sep = " + "), 
                                  fuiktype_info)
    ) %>% 
    mutate(fuiktype_info = gsub(pattern = "\\( \\+ ",
                                replacement = "(",
                                fuiktype_info),
           fuiktype_info = paste0(fuiktype_info, ")"),
           fuiktype_info = gsub(pattern = "\\(\\)",
                                replacement = NA_character_,
                                fuiktype_info)) %>% 
    mutate(fuiktype = paste(fuiktype_base, fuiktype_info),
           fuiktype = gsub(pattern = "NA",
                           replacement = "",
                           fuiktype),
           fuiktype = trimws(fuiktype, which = "both")) %>% 
    mutate(actie = ifelse(grepl(pattern = "Afvangst",
                                x = `Wat wil je melden`,
                                ignore.case = TRUE),
                          "Afvangst",
                          "--AANVULLEN--")) %>% 
    mutate(fuiklengte = case_when(grepl(pattern = "13m",
                                        x = `Wat wil je melden`,
                                        ignore.case = TRUE) ~ "13",
                                  grepl(pattern = "20m",
                                        x = `Wat wil je melden`,
                                  ...
                                  TRUE ~ NA_character_)) %>% 
    ungroup() %>% 
    mutate(fuiktype = case_when(fuiktype == "" & grepl(pattern = "bodem", 
                                                       Opmerkingen,
                                                       ignore.case = TRUE) ~ "salamanderfuik bodem",
                                fuiktype == "" & grepl(pattern = "drijvend", 
                                                       Opmerkingen,
...
afvangsten_per_fuik <- prep_fuiken(data_afvangsten) %>% 
  mutate(L00 = as.numeric(L00),
         ...
         fuikNr2 = as.numeric(gsub(pattern = "Fuik ",
                                   ...
                                   fuikNr))) %>% 
  ungroup()  %>% 
  group_by(Datum, Locatie, fuikNr, Hoek, fuiktype, Tijdstempel) %>% 
...
static_fuikNr <- afvangsten_per_fuik %>% 
  filter(fuikNr == "Fuik 1" & no_fuiken != max_fuikNr) %>% 
  group_by(eventID) %>% 
  add_tally() %>% 
  ungroup() %>% 
  filter(n > 1) %>% 
...
for(e in events_ids){
  ...
  for(f in fuiktypes){
    ...
    max_fuikNr <- max(temp_sub$no_fuiken)
    while(i <= max_fuikNr){
...
afvangsten_per_fuik_recomb <- rbind(static_fuikNr,
                                    dynamic_fuikNr)

test <- afvangsten_per_fuik %>% 
  anti_join(afvangsten_per_fuik_recomb , by = c("eventID", "fuiktype", "L1", "L0", "L00", "L2", "M1", "M2", "AM", "AV")) %>%
  distinct(eventID, fuiktype, fuikNr, L1, L0, L00, L2, M1, M2, AM, AV, .keep_all = TRUE) %>% 
  mutate(Jaar_Tijdstempel = lubridate::year(as.Date(Tijdstempel, format = "%d-%m-%Y %H:%M:%S")),
         fuikNr2 = as.numeric(gsub(pattern = "Fuik ",
                                   replacement = "",
                                   fuikNr))) %>%
  group_by(eventID, fuiktype) %>%
...
afvangsten_per_fuik <- rbind(afvangsten_per_fuik_recomb, test)
...
afvangsten_vr_gbif_old <- read_csv("./output/gbif_stierkikker_vangsten.csv") %>% 
  dplyr::select(-Jaar,
                -`Net verhoogd?`) %>% 
  dplyr::mutate(fuiklengte = as.character(fuiklengte))

afvangsten_vr_gbif_int <- afvangsten_per_fuik %>%
  filter(Jaar > 2018) %>% 
  rename(lat = lat_grid,
         long = long_grid) %>%
...
afvangsten_vr_gbif_int <- afvangsten_vr_gbif_int %>% 
  group_by(rowid) %>% 
  add_tally() %>% 
  ungroup() 

names <- names(afvangsten_vr_gbif_int)
names <- names[!names %in% c("OccurrenceID")]

dups <- afvangsten_vr_gbif_int %>%
  filter(n > 1) %>%
  ungroup()

# Select unique occurrenceID's
unis <- afvangsten_vr_gbif_int %>%
...
# Sometimes the same occurrenceID is used for several occurrences 
# Detect the other rows than the first
...
# Maintain the first row of each OccurrenceID group
# Maintain only the first
dups_catch_fixed <- dups %>% 
  filter(!rowid %in% remaining_dups_catch$rowid) %>% 
  add_tally() %>% 
  ungroup() %>% 
  select(-occurrenceID_old)

testthat::expect_true(n_distinct(dups_catch_fixed$occurrenceID) == nrow(dups_catch_fixed))
...
testthat::expect_true(n_distinct(remaining_dups_catch$occurrenceID) == nrow(remaining_dups_catch))

testthat::expect_true(nrow(dups) == nrow(dups_catch_fixed) + nrow(remaining_dups_catch))

fistools::colcompare(unis, dups_catch_fixed)
fistools::colcompare(unis, remaining_dups_catch)

# Combine the fixed duplicates with the remaining duplicates & the unique records
afvangsten_vr_gbif <- rbind(unis, dups_catch_fixed, remaining_dups_catch) %>%
  arrange(Tijdstempel) %>% 
  select(-n)
...
unis_post_dups <- afvangsten_vr_gbif %>% 
  group_by(occurrenceID) %>%
  add_tally() %>%
  filter(n == 1)

afvangsten_vr_gbif_new <- afvangsten_vr_gbif %>% 
  filter(is.na(occurrenceID)) 

afvangsten_vr_gbif_new$occurrenceID <- UUIDgenerate(n = nrow(afvangsten_vr_gbif_new))

afvangsten_vr_gbif_old <- afvangsten_vr_gbif %>% 
  filter(!is.na(occurrenceID))

# conform classes 
afvangsten_vr_gbif_old <- afvangsten_vr_gbif_old %>% 
  mutate(taxonkey = as.character(taxonkey))

afvangsten_vr_gbif_new <- afvangsten_vr_gbif_new %>% 
  mutate(taxonkey = as.character(taxonkey))

# Combine datasets
afvangsten_vr_gbif <- rbind(afvangsten_vr_gbif_old, afvangsten_vr_gbif_new) %>% 
...
missing_from_new <- afvangsten_vr_gbif %>%
  ...
  filter(!occurrenceID %in% afvangsten_vr_gbif_new$occurrenceID) %>% 
...
data <- data %>% 
  mutate(eventID = paste(format(as.Date(Datum, 
                                        format = "%d-%m-%Y"), 
                                "%d-%m-%Y"), Locatie, sep = "_"))

data_bijvangsten_int <- prep_fuiken(data) %>% 
  filter(!is.na(locationID),
...
species_missing <- data_bijvangsten_int %>% 
  filter(is.na(scientificname)) %>% 
  select(species, scientificname, Opmerkingen)

# test if nrow(species_missing) is larger than 1% of nrow(data_bijvangsten_int)
# and display the distinct of species_missing upon failure
testthat::expect_true(nrow(species_missing) <= 0.01 * nrow(data_bijvangsten_int),
                      info = paste("Missing species names for the following species:",
                                   paste(unique(species_missing$species), collapse = ", ")))

data_bijvangsten_int <- data_bijvangsten_int %>% 
  filter(!is.na(scientificname))

species <- unique(data_bijvangsten_int$scientificname)
...
for(s in species){
  ...
  if("speciesKey" %in% names(temp_lookup)){
    temp_taxonkey <- temp_taxonkey %>% 
      add_row(species = s,
              taxonkey = as.integer(temp_lookup$speciesKey))
  }else{
    temp_taxonkey <- temp_taxonkey %>% 
      add_row(species = s,
              taxonkey = as.integer(temp_lookup$genusKey))
  }
...
table(data_bijvangsten_int$recordedBy,
      data_bijvangsten_int$identifiedBy,
      useNA = "ifany")

events3 <- data_bijvangsten_int %>% 
  ungroup() %>% 
  distinct(Datum,
           Jaar,
           Locatie,
...
dup_bycatch <- data_bijvangsten_int %>% 
  group_by(occurrenceID) %>%
  add_tally() %>%
  filter(n > 1) %>% 
  ungroup() 
...
unis_bycatch <- data_bijvangsten_int %>%
  filter(!occurrenceID %in% occurence_ids) 

data_bijvangsten <- bind_rows(unis_bycatch, dup_bycatch_redux)

remaining_dup_bycatch <- dup_bycatch_redux %>% 
  group_by(occurrenceID) %>%
  add_tally() %>%
  filter(n > 1) %>% 
  ungroup()

testthat::expect_true(nrow(remaining_dup_bycatch) == 0)
...
fistools::colcompare(data_bijvangsten_old, data_bijvangsten)

data_bijvangsten <- data_bijvangsten %>% 
  select(`Wat wil je melden`,
         -`Aantal fuiken (Totaal)`,
         -max_fuikNr.x, 
         -no_fuiken.x, 
         -fuiktype_base, 
         -fuiktype_info, 
...
afvangsten_per_fuik_pre2018 <- read_csv("interim/afvangsten_per_fuik_pre2018.csv") %>% 
  rename(totaal_fuik_corrected = totaal_corrected)
...
afvangsten_per_fuik_overlap <- afvangsten_per_fuik %>% 
  filter(Datum <= max(afvangsten_per_fuik_pre2018$Datum, na.rm = TRUE)) %>% 
  bind_rows(afvangsten_per_fuik_pre2018) %>%
  group_by(eventID) %>% 
  coalesce() %>% 
  mutate(location_temp = as.integer(substr(eventID, 12, 16))) %>% 
  tidylog::distinct(eventID, fuikNr, .keep_all = TRUE) 

afvangsten_per_fuik_postOnly <- afvangsten_per_fuik %>% 
  filter(Datum > max(afvangsten_per_fuik_pre2018$Datum, na.rm = TRUE)) %>%
  mutate(location_temp = as.integer(substr(eventID, 12, 16))) 

afvangsten_per_fuik_combined <- bind_rows(afvangsten_per_fuik_overlap, afvangsten_per_fuik_postOnly)

straalmolen_locations <- afvangsten_per_fuik_combined %>% 
  filter(!is.na(location_temp)) %>% 
  distinct(location_temp) %>%
  pull()

afvangsten_per_fuik_straalmolen <- afvangsten_per_fuik_combined %>% 
  filter(location_temp %in% straalmolen_locations) %>% 
  select(-location_temp) %>% 
  write_csv("interim/afvangsten_per_fuik_straalmolen.csv")

afvangsten_per_fuik_andere <- afvangsten_per_fuik_combined %>% 
  filter(!location_temp %in% straalmolen_locations) %>% 
  select(-location_temp) %>% 
  write_csv("interim/afvangsten_per_fuik_combined.csv")
```
