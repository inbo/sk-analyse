...
eea_grid <- sf::st_read("https://github.com/inbo/aspbo/raw/refs/heads/uat/data/output/UAT_processing/grid/utm1_bel_with_regions.gpkg", layer = "utm1_bel_with_regions") %>% 
  ...
  sf::st_centroid() %>% 
  mutate(lat = sf::st_coordinates(geom)[,2],
         long = sf::st_coordinates(geom)[,1]) %>% 
...
if(sf::st_crs(eea_grid) == sf::st_crs(vijvers_int)){
  vijvers <- sf::st_join(vijvers_int, eea_grid) %>% 
    sf::st_drop_geometry() %>% 
    select(-CELLCODE) %>% 
...
testthat::expect_true(nrow(vijvers) == nrow(vijvers_int) + nrow(vijvers_zndr_coordinaten))
```
...
# set order of colums
afvangsten_per_fuik <- afvangsten_per_fuik %>% 
  select("Tijdstempel", "Invuller 2", "Datum", "Jaar", "Locatie", "actie", "fuiklengte", "Net verhoogd?", "lat", "long", "Type", "locationID", "Organisation", "eventID", "no_fuiken", "fuikNr", "Opmerkingen", "L1", "L0", "L2", "M2", "AV", "AM", "L00", "M1", "Hoek", "fuiktype", "larven_fuik", "totaal_fuik", "correctie_factor", "larven_corrected", "totaal_fuik_corrected") %>% 
...
afvangsten_vr_gbif <- afvangsten_per_fuik %>%
  ...
  tidylog::left_join(afvangsten_vr_gbif_old, by = c("Datum" = "Datum",
                                                    "Locatie" = "Locatie",
                                                    "actie" = "actie",
...
data_bijvangsten <- data %>% 
  ...
  dplyr::select(-Organisation,
                -fuikNr2,
                -max_fuikNr) %>% 
  tidylog::left_join(data_bijvangsten_old, by = c("Datum" = "Datum",
                                                  "Locatie" = "Locatie",
                                                  "actie" = "actie",
                                                  "fuiklengte" = "fuiklengte",
                                                  "Type" = "Type",
...
```{r filter data currently published, eval=FALSE}
afvangsten_vr_gbif <- afvangsten_vr_gbif %>% 
