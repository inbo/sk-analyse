...
uitvoerders <- read_sheet("1_cBgfSjnxakLAQ9uGunDt_-Mk4L0uDx2-qv6cKCzkSk",
                          sheet = "Gebruikers",
                          col_types = "c") %>% 
  select(-Invuller)
```
...
afvangsten_per_fuik <- data_afvangsten %>% 
  ...
  ungroup() %>% 
  group_by(Datum, Locatie, fuikNr, Hoek, fuiklengte, fuiktype) %>% 
...
table(afvangsten_vr_gbif$recordedBy,
      afvangsten_vr_gbif$identifiedBy,
      useNA = "ifany")
```
