...
```{r}
occurrence %<>% mutate(dwc_datasetName = "Invasive Bullfrog occurrences in Flanders, bycatch")
```
...
```{r}
occurrence %<>% mutate(dwc_recordedBy = 'INBO team FIS')
```
...
```{r}
occurrence %<>% mutate(dwc_locality = locatie)
...
```{r}
occurrence %<>% mutate(dwc_identifiedBy = 'INBO FIS')
```
