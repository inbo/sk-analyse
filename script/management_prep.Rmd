...
lifeStages_until_2018_conf <- lifeStages_until_2018_int %>% 
  mutate(lifeStage = case_when(grepl("female", lifeStage2) ~ "AV",
                               ...
                               lifeStage2 %in% c("Juvenieloops", "Juveniel", "metamorph") ~ "M?",
                               lifeStage2 %in% c("adult", "adults") ~ "A?",
                               lifeStage2 %in% c("larvae", "larvae?", "lenght< 5 cm") ~ "L?",
                               TRUE ~ lifeStage2)) %>% 
  pivot_wider(id_cols = gbifID, names_from = lifeStage, values_from = n, values_fn = min, values_fill = NA) %>% 
  group_by(gbifID) %>%
  mutate(larven_fuik = sum(L0, L00, L1 na.rm = TRUE),
         juveniel_fuik = sum(L1, L2, na.rm = TRUE),
